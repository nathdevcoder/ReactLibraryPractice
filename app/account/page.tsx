 ...
 export default function AccountPage() {
  ...
  const applyRole = async () => {
   ...
   return <Box > 
    ...
    {!plan && session?.user.role === 'user' && <Stack direction={'row'} flexWrap='wrap' spacing={4} justifyContent={'center'} my={8}>
        {membershipData.map(mdata=>(
            <Paper key={mdata.name} variant='outlined' square={false}>
                <Box width={230} p={4} pb={2} textAlign={'center'}>
                    ...
                    <Button onClick={()=>upgradePlan(mdata.name)} variant='contained' size='small' fullWidth sx={{bgcolor: mdata.bgColor, color: 'black'}}>Register</Button>
                </Box>
            </Paper> 
        ))}
    </Stack>}
